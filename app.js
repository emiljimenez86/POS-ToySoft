...
function agregarProducto(id) {
  ...
  try {
    if (typeof verificarDisponibilidadProducto === 'function') {
      const disponibilidad = verificarDisponibilidadProducto(producto.nombre, 1);
  ...
  if (productoExistente) {
    ...
    try {
      if (typeof verificarDisponibilidadProducto === 'function') {
        const disponibilidad = verificarDisponibilidadProducto(producto.nombre, productoExistente.cantidad + 1);
      ...
      fila.innerHTML = `
        ...
        <td style="width: 12%">
          <div class="input-group input-group-sm">
            <button class="btn btn-outline-light btn-sm px-1" onclick="cambiarCantidad(${item.id}, '${mesa}', ${item.ronda}, -1)">-</button>
            <input type='number' class='form-control form-control-sm bg-dark text-white border-light text-center' 
                   value='${item.cantidad}' min='1'
                   style="width: 40px;"
                   onchange='actualizarCantidad(this, ${item.id}, "${mesa}", ${item.ronda})' />
            <button class="btn btn-outline-light btn-sm px-1" onclick="cambiarCantidad(${item.id}, '${mesa}', ${item.ronda}, 1)">+</button>
          </div>
        </td>
...
function cambiarCantidad(id, mesa, ronda, cambio) {
  const pedido = mesasActivas.get(mesa);
  if (!pedido || !pedido.items) return;

  const producto = pedido.items.find(p => p.id === id && p.ronda === ronda && p.estado !== 'en_cocina');
  if (producto) {
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const inputValorDomicilio = document.getElementById('valorDomicilio');
  if (inputValorDomicilio) {
    inputValorDomicilio.addEventListener('input', () => {
      if (mesaSeleccionada) {
        actualizarTotal(mesaSeleccionada);
      }
    });
